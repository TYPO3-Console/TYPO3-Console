...
class ConsoleBootstrap extends Bootstrap
{
    ...
    public function baseSetup($pathPart = '')
    {
        ...
        if (is_callable([__CLASS__, 'initializeComposerClassLoader'])) {
...
//		error_reporting(E_ALL & ~(E_STRICT | E_NOTICE));
    ...
    public function initializePackageManagement($packageManagerClassName = 'Helhum\\Typo3Console\\Package\\UncachedPackageManager')
    {
        ...
        // @deprecated in 6.2, will be removed in 7.0
        if (is_callable([$packageManager, 'injectClassLoader'])) {
        ...
        // required since 7.4
        if (is_callable([$dependencyResolver, 'injectDependencyOrderingService'])) {
    ...
    {
        // @deprecated in 6.2, will be removed in 7.0
        if (is_callable([__CLASS__, 'disableCoreAndClassesCache'])) {
    ...
    {
        if (is_callable([$this, 'initializeErrorHandling'])) {
